    ...
    <nav class="nav nav-pills nav-fill flex-column">
      ...
      <c:choose>
      		<c:when test="${toActive == 5}">
      			<a class="nav-link active" href="${statistics}">Рузультати опитування</a>
      		</c:when>
      		<c:otherwise>
      			<a class="nav-link" href="${statistics}">Рузультати опитування</a>
      		</c:otherwise>
      	</c:choose>
      	<sec:authorize access="hasRole('ROLE_ADMIN')">
	      	<c:choose>
	      		<c:when test="${toActive == 6}">
	      			<a class="nav-link active mt-5" href="${adminStats}">Рузультати опитування (адмін)</a>
	      		</c:when>
	      		<c:otherwise>
	      			<a class="nav-link mt-5" href="${adminStats}">Рузультати опитування (адмін)</a>
	      		</c:otherwise>
	      	</c:choose>
        </sec:authorize>
    </nav>
